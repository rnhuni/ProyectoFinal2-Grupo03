...
def start_report_polling():
    with app.app_context():
        ReportService().poll_queue()

def start_task_polling():
    with app.app_context():
        TaskService().poll_queue()

threading.Thread(target=start_report_polling, daemon=True).start()
threading.Thread(target=start_task_polling, daemon=True).start()

if __name__ == '__main__':
    print("ServicioReporte is running on 5000")
