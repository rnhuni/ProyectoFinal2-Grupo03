...
class TestActiveSubscriptionExists:
    @pytest.fixture(autouse=True)
    def setup_method(self, mocker):
        self.mock_query = mocker.patch.object(session, 'query')

    def test_active_subscription_exists_by_id(self):
        mock_subscription = MagicMock()
        self.mock_query.return_value.get.return_value = mock_subscription

        command = ActiveSubscriptionExists(id="test-id")
    ...
    def test_active_subscription_does_not_exist_by_id(self):
        self.mock_query.return_value.get.return_value = None

        command = ActiveSubscriptionExists(id="nonexistent-id")
        exists = command.execute()
    ...
    def test_active_subscription_exists_by_client_id_and_status(self):
        mock_subscription = MagicMock()
        self.mock_query.return_value.filter_by.return_value.first.return_value = mock_subscription

        command = ActiveSubscriptionExists(client_id="client-1", status="active")
        exists = command.execute()
    ...
    def test_active_subscription_does_not_exist_by_client_id_and_status(self):
        self.mock_query.return_value.filter_by.return_value.first.return_value = None

        command = ActiveSubscriptionExists(client_id="client-1", status="inactive")
        exists = command.execute()
    ...
    def test_active_subscription_invalid_data(self):
        command = ActiveSubscriptionExists()
