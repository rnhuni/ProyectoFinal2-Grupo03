import pytest
from unittest.mock import MagicMock, patch
from ServicioCanal.models.model import session
from ServicioCanal.models.session import Session
from ServicioCanal.commands.session_claim import ClaimSession

class TestClaimSession:
    @pytest.fixture(autouse=True)
    def setup_method(self, mocker):
        self.mock_query = mocker.patch.object(session, 'query')
        self.mock_query_instance = self.mock_query.return_value
        self.mock_query_instance.filter_by.return_value.first.return_value = None
        self.mock_commit = mocker.patch.object(session, 'commit')
        self.mock_rollback = mocker.patch.object(session, 'rollback')

    def test_claim_session_success(self):
        mock_session = MagicMock(spec=Session)
        self.mock_query_instance.filter_by.return_value.first.return_value = mock_session

        command = ClaimSession(
            session_id="session-1",
            user_id="user-123",
            user_name="John Doe",
            user_type="agent"
        )
        result = command.execute()

        assert result == mock_session
        assert mock_session.assigned_to_id == "user-123"
        assert mock_session.assigned_to_name == "John Doe"
        assert mock_session.assigned_to_type == "agent"
        assert mock_session.status == "ASSIGNED"
        self.mock_commit.assert_called_once()

    def test_claim_session_not_found(self):
        self.mock_query_instance.filter_by.return_value.first.return_value = None

        command = ClaimSession(
            session_id="nonexistent-id",
            user_id="user-123",
            user_name="John Doe",
            user_type="agent"
        )
        with pytest.raises(ValueError, match="Session not found"):
            command.execute()

        self.mock_commit.assert_not_called()
        self.mock_rollback.assert_called_once()

    def test_claim_session_no_id(self):
        command = ClaimSession(
            session_id=None,
            user_id="user-123",
            user_name="John Doe",
            user_type="agent"
        )
        with pytest.raises(ValueError, match="Session ID is required"):
            command.execute()

        self.mock_commit.assert_not_called()
        self.mock_rollback.assert_not_called()

    def test_claim_session_database_error(self):
        self.mock_query_instance.filter_by.side_effect = Exception("Database error")

        command = ClaimSession(
            session_id="session-1",
            user_id="user-123",
            user_name="John Doe",
            user_type="agent"
        )
        with pytest.raises(Exception, match="Database error"):
            command.execute()

        self.mock_commit.assert_not_called()
        self.mock_rollback.assert_called_once()
